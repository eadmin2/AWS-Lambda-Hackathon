...
const UserDetailModal: React.FC<UserDetailModalProps> = ({ isOpen, onClose, user, onSave, loading, onUpdateCredits, canEditCredits }) => {
  ...
  const [creditsEdit, setCreditsEdit] = useState<{ [paymentId: string]: number }>({});
  const [creditsSaving, setCreditsSaving] = useState<{ [paymentId: string]: boolean }>({});
          ...
          <ul className="mt-1 text-gray-900 max-h-32 overflow-y-auto text-sm">
            {user.user_documents && user.user_documents.length > 0 ? (
              user.user_documents.map(doc => (
                <li key={doc.id} className="py-1 border-b last:border-b-0 flex justify-between items-center">
                  <span>{doc.file_name}</span>
                  {doc.uploaded_at && <span className="text-xs text-gray-500 ml-2">{doc.uploaded_at}</span>}
                </li>
              ))
            ) : (
          ...
          <ul className="mt-1 text-gray-900 text-sm">
            {user.payments && user.payments.length > 0 ? (
              user.payments.map((pay: any, idx: number) => (
                <li key={pay.id || idx} className="py-1 border-b last:border-b-0">
                  <div>Status: {pay.subscription_status || 'None'}</div>
                  {pay.subscription_end_date && <div>Ends: {pay.subscription_end_date}</div>}
