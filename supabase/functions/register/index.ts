...
Deno.serve(async (req) => {
  ...
  if (!email || !fullName) {
    return new Response(JSON.stringify({ error: "Missing required fields: email, fullName" }), { status: 400, headers: corsHeaders });
  }
  ...
  const welcomeEmailRes = await fetch(`${websiteUrl}/functions/v1/send-email`, {
    method: "POST",
  ...
  if (!welcomeEmailRes.ok) {
    ...
    return new Response(JSON.stringify({ error: "User registered, but failed to send welcome email", details: err }), { status: 500, headers: corsHeaders });
  }

  return new Response(JSON.stringify({ success: true, message: "Registration initiated. Check your email for a login link or OTP." }), { status: 200, headers: corsHeaders });
}); 