...
const createTextChunks = (text, pageNumber = 1) => {
  ...
  for (const sentence of sentences) {
    ...
    if (potentialChunk.length > CHUNK_CONFIG.maxChunkSize && currentChunk.length > CHUNK_CONFIG.minChunkSize) {
      // Save current chunk
      chunks.push({
        index: chunkIndex++,
      ...
      const overlapWords = words.slice(-Math.floor(CHUNK_CONFIG.overlapSize / 6)); // rough estimate
...
const handleSNSNotification = async (event, requestId) => {
  try {
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({ message: "SNS notification processed", requestId })
    };
...
const handleS3Upload = async (event, requestId) => {
  try {
    ...
    console.log(`Processing uploaded file: ${filePath} from AWS S3 bucket: ${bucketName}`);
    
    // Validate bucket
    if (bucketName !== AWS_S3_BUCKET) {
      console.error(`Unexpected bucket: ${bucketName}, expected: ${AWS_S3_BUCKET}`);
      return { statusCode: 400, body: JSON.stringify({ error: "Invalid bucket", requestId }) };
    }
    ...
    if (pathParts.length === 1) {
      ...
      return { statusCode: 400, body: JSON.stringify({ error: "Invalid file path format", requestId }) };
    }
    ...
    if (!uuidRegex.test(userId)) {
      ...
      return { statusCode: 400, body: JSON.stringify({ error: "Invalid user ID format", requestId }) };
    }
    ...
    return await startTextractProcessing(bucketName, filePath, documentId, userId, requestId);
    
  } catch (error) {
    console.error("S3 upload handler error:", error);
    return { statusCode: 500, body: JSON.stringify({ error: error.message, requestId }) };
  }
};

// Start Textract processing with AWS S3
const startTextractProcessing = async (bucketName, filePath, documentId, userId, requestId) => {
  try {
    console.log(`[DEBUG] Starting Textract for AWS S3: s3://${bucketName}/${filePath}`);
    ...
    console.log(`[DEBUG] S3 object confirmed: ${headResult.ContentLength} bytes, ${headResult.ContentType}`);
    ...
    console.log(`[SUCCESS] Started Textract job ${awsJobId} for document ${documentId}`);
...
const handleTextractCompletion = async (awsJobId) => {
  try {
    ...
    const processedData = await processTextractForRAG(result.Blocks, jobData.document_id);
